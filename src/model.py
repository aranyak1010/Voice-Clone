...
class Decoder(nn.Module):
    def __init__(self, config):
        ...
        self.prenet_dim = 256
        self.max_decoder_steps = config['decoder_max_step']
        ...
        self.attention_rnn = nn.LSTMCell(
            self.prenet_dim + config['encoder_embedding_dim'],
    ...
    def get_go_frame(self, memory):
        B = memory.size(0)
        go_frame = memory.new_zeros(B, self.n_mel_channels * self.n_frames_per_step)
        return go_frame
...
class Tacotron2(nn.Module):
    def __init__(self, config):
        ...
        self.embedding = nn.Embedding(
            len(" abcdefghijklmnopqrstuvwxyz.,!?;:"), config['encoder_embedding_dim'])
        std = np.sqrt(2.0 / (len(" abcdefghijklmnopqrstuvwxyz.,!?;:") + config['encoder_embedding_dim']))
        val = np.sqrt(3.0) * std
...
class Postnet(nn.Module):
    def __init__(self, config):
        ...
        self.convolutions.append(
            nn.Sequential(
                ConvNorm(config['n_mel_channels'], 512,
        ...
        self.convolutions.append(
            nn.Sequential(
                ConvNorm(512, config['n_mel_channels'],
                         kernel_size=5, stride=1,
                         padding=2,
